...
function watchUsers() {
  ...
  state.unsubscribe = onValue(ref(state.db, "users"), (snapshot) => {
    ...
    state.users.sort((a, b) => Number(isOnline(b)) - Number(isOnline(a)) ||
      Math.max(b.lastSeenAt, b.lastLoginAt) - Math.max(a.lastSeenAt, a.lastLoginAt));
    renderUsers();
    setSync("Live", "live");
  }, (error) => {
    console.error(error);
...
function normalizeUser(uid, data) {
  return {
    ...
    lastSeenAt: numberValue(data.lastSeenAt)
  };
}
...
function userRow(user) {
  ...
  const badges = [
    `<span class="badge ${isOnline(user) ? "online" : "offline"}" data-online-badge="${user.uid}">${isOnline(user) ? "ONLINE" : "OFFLINE"}</span>`,
  ...
  return `<tr>
    ...
    <td><span class="presence-text ${isOnline(user) ? "presence-now" : ""}" data-presence="${user.uid}">${escapeHtml(presenceText(user))}</span></td>
...
function updateStats() {
  ...
  $("#onlineUsers").textContent = state.users.filter(isOnline).length;
}
...
function updatePresenceIndicators() {
  document.querySelectorAll("[data-online-badge]").forEach((node) => {
    const user = findUser(node.dataset.onlineBadge);
    if (!user) return;
  ...
  document.querySelectorAll("[data-presence]").forEach((node) => {
    const user = findUser(node.dataset.presence);
    if (!user) return;
    node.textContent = presenceText(user);
    node.classList.toggle("presence-now", isOnline(user));
  });
}
...
function updateModal(user) {
  ...
  $("#modalPresence").textContent = presenceText(user);
...
function isOnline(user) { return Boolean(user?.online && user.lastSeenAt > Date.now() - 120000); }
function presenceText(user) { return isOnline(user) ? "Online now" : (user.lastSeenAt > 0 ? formatDate(user.lastSeenAt) : "Not available"); }
